...
for forbidden_prefix in 'providers.local/' 'integrations.local/' 'data/' 'connectors/'; do
...
if git grep -n -I -E '(gh[pousr]_[A-Za-z0-9]{20,}|sk-[A-Za-z0-9_-]{16,}|-----BEGIN [A-Z ]*PRIVATE KEY-----|Bearer[[:space:]]+[A-Za-z0-9._~+/-]{20,})' -- . ':!tests' >/tmp/search-governor-secret-scan.txt; then
